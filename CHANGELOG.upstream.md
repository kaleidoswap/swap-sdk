...
This file preserves the release notes inherited from
[`boltz-rust`](https://github.com/SatoshiPortal/boltz-rust) when KaleidoSwap SDK
forked it. The versions below belong to the upstream `boltz-client` package,
not to KaleidoSwap SDK. They are archived verbatim from the fork point so the
two projects' independent version lines cannot be confused.
