...
nothing more. On a `0.x` version a caret range does exactly that. `^0.10.0`
means `>=0.10.0 <0.11.0`: it picks up `0.10.1` and never crosses into `0.11.0`,
where a breaking change can land. Python's compatible-release operator means the
same thing.

```json
"@kaleidorg/swap-sdk": "^0.10.0"
```
...
lockfile already makes a build reproducible. An exact pin such as `"0.10.0"`
